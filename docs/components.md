...
3. If execution succeeds but handoff to event dispatch fails, the command entry is retried (which can repeat command handling).
4. Completion means responsibility moved from command processing to event dispatching.
...
**Model:**
The main models are `EventEnvelope` (in-flight event + metadata) and `EventEntry` (durable queued form used for retries).
...
3. Direct dispatch can observe partial delivery if one subscriber fails after others have succeeded; retries can re-invoke already-successful subscribers.
4. Completion means responsibility moved from event dispatching to subscriber processing.
...
4. Completion means responsibility moved from the outbox to the external transport.
