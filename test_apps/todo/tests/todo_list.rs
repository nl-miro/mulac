...
async fn complete_todo(base_url: &str, todo_id: Uuid) {
    let response = utils::client().post(format!("{base_url}/api/todos/{todo_id}/complete")).send().await.unwrap();
...
async fn list_todos_returns_all_todos() {
    ...
    let response = utils::client().get(format!("{base_url}/api/todos")).send().await.unwrap();
...
async fn list_todos_filters_by_status() {
    ...
    let active_response = utils::client().get(format!("{base_url}/api/todos?status=active")).send().await.unwrap();
    let active_body = active_response.json::<serde_json::Value>().await.unwrap();
    let active = active_body["items"].as_array().unwrap();
    assert_eq!(active.len(), 0);

    let completed_response = utils::client().get(format!("{base_url}/api/todos?status=completed")).send().await.unwrap();
    let completed_body = completed_response.json::<serde_json::Value>().await.unwrap();
