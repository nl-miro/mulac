...
use serde_json::json;
use utils::{
    STATUS_COMPLETED, fetch_command_entries, fetch_timelines_for_user, retry_fan_out,
    start_test_app,
};
use uuid::Uuid;
...
async fn tweet_posted_fans_out_to_followers() {
    ...
    let fanout_cmds: Vec<_> = fetch_command_entries(&pool)
        .into_iter()
        .filter(|c| c.command_type == "FanOutTweet")
        .collect();
    assert_eq!(fanout_cmds.len(), 1);
    assert_eq!(fanout_cmds[0].status, STATUS_COMPLETED);
}
...
async fn fan_out_is_idempotent() {
    ...
    let fanout_cmds: Vec<_> = fetch_command_entries(&pool)
        ...
        .filter(|c| c.command_type == "FanOutTweet")
        .collect();
    assert_eq!(fanout_cmds.len(), 2);
    assert!(fanout_cmds.iter().all(|c| c.status == STATUS_COMPLETED));
}
